...
def GrabXlsData(bookname):
	...
	questIndex = worksheet.cell(2, 2).value
	displayName = worksheet.cell(2, 3).value
	questDesc = worksheet.cell(2, 4).value
	energyCost = worksheet.cell(2, 5).value
	minLevel = worksheet.cell(2, 6).value
	stageFSM = worksheet.cell(2, 7).value
	dungeonType = worksheet.cell(2, 11).value
	...
	rList = questname, questIndex, displayName, questDesc, energyCost, minLevel, stageFSM, dungeonType
	return rList
...
#write title
title = 'questname' ,'questIndex' ,'displayName' ,'questDesc' ,'energyCost' ,'minLevel' ,'stageFSM' ,'dungeonType' 
for j in range (9):
