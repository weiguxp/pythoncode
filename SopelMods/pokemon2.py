...
def startgame(bot, trigger):
	...
	if 'pokedex' in globals():
		...
		myBot = bot
		pokedex = dict()
		BotBlueMsg ('DovantorrPokemon v0.3 (BuildNumber %g): type .pokehelp for commands' % GetBuildNumber())
		BotBlueMsg ('==Gotta catch them all!!==')


@commands('pokehelp')
def pokehelp(bot, trigger):
	bot.say ('Available commands are .create .poke .perk .feed .evolve .fight (optional: target username) .gexplore(group explore) .explore .explorewilds .rename')
...
def createpokemon(bot, trigger):
	if trigger.nick in pokedex:
		...
		newPokemon = Pokemon(trigger.group(2), RollDice(7,16,1))
...
def cEnemy(bot, trigger):
	...
	evilPokemon = WildPokemon1(genEnemyLevel, RollDice(3,(30+genEnemyLevel)))

	bot.say(str(evilPokemon))

@commands('gexplore')
def cEnemyg(bot, trigger):
	...
	evilPokemon = WildPokemon1(genEnemyLevel, RollDice(3,(40+genEnemyLevel*2)))
...
class NPCPokemon(Pokemon):
	...
	def MakeWild(self):
		...
		if random.randint(0,100) < 20:
			self.leechseed = random.randint(20,30)
			self.pokename = '[Leechseed]' + self.pokename
			self.killxp = int(self.killxp * 1.2)
		if random.randint(0,100) < 20:
			self.block = random.randint(10,15)
			self.pokename = '[ShieldWall]' + self.pokename
			self.killxp = int(self.killxp * 1.2)
		if random.randint(0,100) < 15:
			self.crit = random.randint(40,60)
			self.pokename = '[MegaCrit]' + self.pokename
			self.killxp = int(self.killxp * 1.2)
...
class WildPokemon1(NPCPokemon):
	def __init__(self, level, hp):
		...
		self.level = level
		self.weapon = 2, random.randint(1,12+(self.level))
		self.maxhp = hp
		self.hp = hp
